...
if (isset($_GET['delete_id'])) {
    ...
    if ($stmt->execute()) {
        header("Location: admin_dashboard.php?page=flavors");
        exit();
    } else {
        $error = "Không thể xóa $attribute_name. Có thể đang được sử dụng.";
    }
    $stmt->close();
}
...
<div class="admin-content">
    <div class="admin-page-header">
        <h1 class="admin-page-title"><i class="fas fa-cube"></i> Quản lý <?php echo $attribute_name; ?></h1>
    </div>
        ...
        <form method="POST" class="admin-add-form">
            <div class="admin-form-row">
                <div class="admin-form-group">
                    ...
                    <input type="text" id="new_name" name="new_name" placeholder="Ví dụ: Cốt Vani + Mứt Dâu" required>
                </div>
        ...
        <table class="admin-table">
            ...
            <tbody>
                <?php if ($result && $result->num_rows > 0): ?>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="3">Chưa có <?php echo $attribute_name; ?> nào.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
    <div id="editModalFlavor" class="edit-modal" style="display:none;">
...
<script>
(function() {
    var modal = document.getElementById("editModalFlavor");
    if (!modal) return;
